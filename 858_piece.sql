SET DEFINE OFF;
Insert into FC_REF_PIECE
   (IDE_POSTE, IDE_REF_PIECE, IDE_ORDO, COD_BUD, COD_TYP_PIECE, 
    IDE_PIECE, COD_REF_PIECE, VAR_TIERS, IDE_TIERS, IDE_GEST, 
    FLG_CPTAB, MT_DB, MT_CR, DAT_DER_MVT, FLG_SOLDE, 
    IDE_JAL, IDE_ECR, IDE_LIG, DAT_CRE, UTI_CRE, 
    DAT_MAJ, UTI_MAJ, TERMINAL, IDE_PLAN_AUX, IDE_CPT_AUX, 
    IDE_DEVISE, VAL_TAUX, MT_DEV, NUM_LIG_TIERS)
 Values
   ('858', 118252, NULL, NULL, NULL, 
    NULL, 'CHQ N?8847767  /  DIAMOND BANK SIEGE', 'T2012', '9705916C', '2020', 
    'O', 300035, 0, TO_DATE('10/01/2020', 'DD/MM/YYYY'), 'N', 
    'T29', 4, 1, TO_DATE('10/07/2020 12:34:05', 'DD/MM/YYYY HH24:MI:SS'), 'PIAF_ADM', 
    TO_DATE('10/07/2020 12:34:05', 'DD/MM/YYYY HH24:MI:SS'), 'PIAF_ADM', 'DSI0214B', NULL, NULL, 
    NULL, NULL, 300035, NULL);
COMMIT;
