...
spool c:\Sygacut\Exce_Sygacut_mois_TG.sql
...
(select transf_pc_code,bord_rgl_num from cut_transfert where transf_date like '%&&date_reglement%' and transf_param_code not like '%MT900%' and transf_pc_code in (select ide_poste from rm_poste@asterndir where ide_typ_poste in ('TG')) 
minus
select transf_pc_code,bord_rgl_num from cut_transfert_2020 where transf_date like '%&&date_reglement%' and transf_param_code not like '%MT900%' and transf_pc_code in (select ide_poste from rm_poste@asterndir where ide_typ_poste in ('TG')) );
commit;
...
minus
 ...
 and transf_param_code not in ('RMT900') and TRANSF_date like '%&&date_reglement%' and transf_pc_code in (select ide_poste from rm_poste@asterndir where ide_typ_poste in ('TG')) --,'TG','T','P'))
 order by 1;
...
@c:\Sygacut\Exce_Sygacut_mois_TG.sql
