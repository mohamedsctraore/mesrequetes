...
spool c:\Sygacut\Exce_Sygacut_mois_CG.sql

insert into cut_transfert_2020 select * from cut_transfert where (TRANSF_date like '%&&date_reglement%') and transf_param_code not like '%MT900%'  and transf_pc_code like '5%'
and (transf_pc_code,bord_rgl_num) in
(select transf_pc_code,bord_rgl_num from cut_transfert where transf_date like '%&&date_reglement%' and transf_param_code not like '%MT900%' and transf_pc_code like '5%'
minus
select transf_pc_code,bord_rgl_num from cut_transfert_2020 where transf_date like '%&&date_reglement%' and transf_param_code not like '%MT900%' and transf_pc_code like '5%');
...
minus
 ...
 and transf_param_code not in ('RMT900') and TRANSF_date like '%&&date_reglement%' and transf_pc_code like '5%' --,'TG','T','P'))
 order by 1;
...
@c:\Sygacut\Exce_Sygacut_mois_CG.sql
