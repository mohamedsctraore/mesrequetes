...
WHERE a.IDE_GEST between '2016' and '2019'
AND a.COD_SENS = 'D' 
----AND a.ide_jal in ('JPECDEPBG')   -------and ide_jal='JODPGAP'
AND a.ide_cpt in ('436.82')
------ and a.dat_maj like '%08/03/2021%' or a.dat_maj like '%09/03/2021%'
---and e.LIBN like '%OITH%'
---AND a.ide_lig_exec like  ('%71111%')
AND a.ide_poste in (select ide_poste from rm_poste where ide_poste_centra='631' or ide_poste='631')
...
AND a.dat_ecr between '01/01/2016' and '31/12/2019'
UNION 
...
SELECT * FROM MOHAMED_COMPTE2022 ORDER BY ide_gest,ide_cpt,(DEBIT+CREDIT),dat_ecr,ide_poste,ide_tiers,ide_jal,ide_ecr
;
