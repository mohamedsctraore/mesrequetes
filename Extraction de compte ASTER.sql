...
AND a.ide_cpt in ('531222')
------ and a.dat_maj like '%08/03/2021%' or a.dat_maj like '%09/03/2021%'
---and e.LIBN like '%OITH%'
---AND a.ide_lig_exec like  ('%71111%')
--AND a.ide_poste in (select ide_poste from rm_poste where ide_poste_centra='435' or ide_poste='435')
...
AND a.dat_ecr between '14/04/2025' and '14/04/2025'
UNION ALL
...
SELECT * FROM MOHAMED_COMPTE2022 ORDER BY ide_cpt,ide_gest,ABS((DEBIT+CREDIT)),dat_ecr,ide_poste,ide_tiers,ide_jal,ide_ecr;

SELECT * FROM MOHAMED_COMPTE2022 WHERE IDE_JAL <> 'JCENTRAL' ORDER BY ide_cpt,ide_gest,dat_ecr,ide_poste,ide_tiers,ide_jal,ide_ecr;

SELECT *
FROM MOHAMED_COMPTE2022 
WHERE (ide_gest, ide_cpt, libelle_compte, ide_poste, libelle, abs(debit)) not in
(
SELECT ide_gest, ide_cpt, libelle_compte, ide_poste, libelle, abs(credit) FROM MOHAMED_COMPTE2022
) 
...
SELECT *
FROM MOHAMED_COMPTE2022 
WHERE (ide_gest, ide_cpt, libelle_compte, ide_poste, libelle, abs(credit) ) not in
(
SELECT ide_gest, ide_cpt, libelle_compte, ide_poste, libelle, abs(debit) FROM MOHAMED_COMPTE2022
) 
;
