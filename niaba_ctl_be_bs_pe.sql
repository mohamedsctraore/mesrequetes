...
select 'BS' cible,ide_poste,ide_cpt,sum(decode(cod_sens,NIABA.ASTER_get_COD_SENS (ide_poste,ide_cpt),mt_dev,-mt_dev)) mt_dev, ide_devise
from fc_ligne 
where ide_gest='2024' 
and flg_cptab='O' 
and ide_cpt in (select ide_cpt from fn_compte where cod_typ_be='P')
and ide_poste in (select unique ide_poste from rm_poste where ide_typ_poste in ('P','PC'))
...
union
select 'BE',ide_poste,ide_cpt,sum(decode(cod_sens,NIABA.ASTER_get_COD_SENS (ide_poste,ide_cpt),mt_dev,-mt_dev)) mt_dev, ide_devise 
from fc_ligne 
where ide_gest='2025' 
and ide_jal='TREP' 
and ide_cpt in (select ide_cpt from fn_compte where cod_typ_be='P')
...
select ide_poste,ide_cpt,ide_devise from ctl_bs_be 
--where ide_poste not like '3%'
group by ide_poste,ide_cpt,ide_devise having sum(decode(cible,'BS',mt_dev,-mt_dev))<>0
order by ide_poste,ide_cpt;

select ide_poste, ide_cpt,ide_devise, sum(decode(cible,'BS',mt_dev,-mt_dev))
from ctl_bs_be
--where Ide_Poste In 
--(Select Ide_Poste from Rm_Poste where Ide_Typ_Poste In ('P','PC'))
group by ide_poste,ide_cpt,ide_devise
HAVING sum(decode(cible,'BS',mt_dev,-mt_dev)) <> 0
order by ide_poste, ide_cpt;
