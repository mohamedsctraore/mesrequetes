...
loop 
        ...
        select unique b.libn as IDE_POSTE,
                      nvl((select to_char(max(dat_jc)) from fc_calend_hist where ide_gest = '2025' and ide_poste = v_ide_poste  and cod_ferm = 'C'),'Pas de journee clotur»e')  as ETAT_CLOTURE,
                      nvl((select to_char(max(dat_jc)) from fc_calend_hist where ide_gest = '2025' and ide_poste = v_ide_poste and cod_ferm = 'O'),'Pas de journee ouverte') as ETAT_OUVERT,
                      nvl((select to_char(max(dat_jc)) from fc_ecriture where ide_gest = '2025' and ide_poste = v_ide_poste and flg_cptab = 'O'), 'Aucune comptabilisation') as ETAT_COMPTABILISE,
                      nvl((select unique cod_ferm from fc_calend_hist where ide_gest = '2025' and ide_poste = v_ide_poste and cod_ferm = 'O'),'C') CODE,
                      nvl((select count(ide_poste) from fc_calend_hist where ide_gest = '2025' and cod_ferm = 'O' and ide_poste = v_ide_poste), 0) JRNES_OUVERTES
