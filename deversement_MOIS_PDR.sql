...
spool c:\Sygacut\Exce_Sygacut_mois_PDR.sql
...
(select transf_pc_code,bord_rgl_num from cut_transfert where transf_date like '%&&date_reglement%' and transf_param_code not like '%MT900%' and transf_pc_code in (select ide_poste from rm_poste@asterndir where ide_typ_poste in ('PDR')) 
minus
select transf_pc_code,bord_rgl_num from cut_transfert_2020 where transf_date like '%&&date_reglement%' and transf_param_code not like '%MT900%' and transf_pc_code in (select ide_poste from rm_poste@asterndir where ide_typ_poste in ('PDR')) );
commit;

select 'alter session set nls_date_format = ''DD/MM/YYYY'';' from dual;
select '/' from dual;

 select distinct 'execute CUT.P_REPrise_DEVERS_ASTERNDIR2 ('''||transf_pc_code||''','''||substr('&&date_reglement',4,4)||''','''||to_date(transf_date,'DD/MM/RRRR')||''')' 
...
minus
 ...
 and transf_param_code not in ('RMT900') and TRANSF_date like '%&&date_reglement%' and transf_pc_code in (select ide_poste from rm_poste@asterndir where ide_typ_poste in ('PDR')) --,'TG','T','P'))
 order by 1;
...
@c:\Sygacut\Exce_Sygacut_mois_PDR.sql
