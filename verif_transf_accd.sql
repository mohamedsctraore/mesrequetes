...
and dat_ecr between '01/12/2024' and '31/01/2025'
...
and a.dat_jc <= '31/01/2025'
--and b.dat_ref between '01/12/2024' and '31/05/2024'
and ( regexp_like (substr(libn, -8), '^[0-9]{2}\/[0-9]{2}\/[0-9]{2}$') and  to_date(substr(libn, -8)) between '01/12/2024' and '31/01/2025')
...
and a.dat_jc > '31/01/2025'
--and b.dat_ref between '01/12/2024' and '31/05/2024'
and ( regexp_like (substr(libn, -8), '^[0-9]{2}\/[0-9]{2}\/[0-9]{2}$') and  to_date(substr(libn, -8)) between '01/01/2024' and '31/01/2025')
and (a.ide_poste like '2%C' or a.ide_poste = '507')
--and a.ide_poste in (select ide_poste from piaf_adm.rm_poste where ide_poste_centra = '501C')
group by a.ide_nd_emet, a.ide_poste, b.ide_cpt, cod_sens
...
--and spec3 is not null
and ide_jal <> 'JTRANSFERT'
and dat_ecr between '01/01/2025' and '31/01/2025'
and (ide_poste like '2%C' or ide_poste = '507')
--and ide_poste in (select ide_poste from piaf_adm.rm_poste where ide_poste_centra = '501C')
group by ide_poste, ide_cpt, spec3, cod_sens
...
and a.dat_jc <= '31/01/2025'
--and b.dat_ref between '01/12/2024' and '31/05/2024'
and ( regexp_like (substr(libn, -8), '^[0-9]{2}\/[0-9]{2}\/[0-9]{2}$') and  to_date(substr(libn, -8)) between '01/01/2025' and '31/01/2025')
and (a.ide_poste like '2%C' or a.ide_poste = '507')
--and a.ide_poste in (select ide_poste from piaf_adm.rm_poste where ide_poste_centra = '501C')
group by a.ide_nd_emet, a.ide_poste, b.ide_cpt, cod_sens;
...
select compte, emetteur, montant_emis_debit emis_debit, montant_emis_credit emis_credit, 
       ...
       (montant_emis_debit + montant_emis_credit)) anomalie
from tableau_transfert
where ((montant_bonne_periode_debit + montant_bonne_periode_credit + sept_pris_en_octobre_debit + sept_pris_en_octobre_credit + montant_non_denoue_debit + montant_non_denoue_credit) 
       - 
       (montant_emis_debit + montant_emis_credit)) <> 0
and emetteur is not null
order by emetteur
;
...
from fc_ecriture
where ide_gest  = '2024'
and ide_poste = '&&emetteur'
and dat_jc between '01/12/2024' and '31/01/2025'
)
and spec3 = '&&destinataire'
...
from fc_ecriture
where ide_gest  = '2024'
and ide_poste = '&&destinataire'
and ide_nd_emet = '&&emetteur'
and ide_jal = 'JTRANSFERT'
)
and dat_ref between '01/12/2024' and '31/01/2025'
and ide_cpt like '391%'
;

--------------------------- TRANSFERT SPECIFIQUE AU SIEGE ---------------------------------------
...
from fc_ecriture
where ide_gest  = '2024'
and libn like 'Transfert 507 du 12/12/24'
and ide_jal = 'JTRANSFERT'
)
and ide_cpt like '391%'
;
