...
WHERE   a.IDE_GEST in ('2022')
AND a.COD_SENS = 'D' 
----AND a.ide_jal in ('JPECDEPBG')   -------and ide_jal='JODPGAP'
AND a.ide_cpt in ('5811216')
AND a.ide_poste='205'
...
AND a.dat_ecr between '01/01/2022' and '31/12/2022'
UNION 
...
---AND a.ide_lig_exec like  ('%71111%')
AND a.ide_poste = '507'
AND observ like '%N%V%205%'
AND observ not like '%2054%' AND observ not like '%2057%' AND observ not like '%2055%'
--AND a.ide_poste in (select ide_poste from rm_poste where ide_poste_centra='510C' or ide_poste='510C')
---and a.ide_tiers in ('PAIMSC','253') ---- is not null
---AND a.spec2='474.3'
--AND a.spec1='7233'
