...
AND a.ide_gest='2022'
AND ide_jal = 'TREP'
--AND a.ide_poste in (SELECT ide_poste FROM rm_poste WHERE ide_poste_centra='657C' or ide_poste='657C')
AND dat_ecr <= '31/07/2022' --BETWEEN '01/01/2022' AND '31/12/2022'
...
SELECT ide_poste_centra, ide_cpt, libn, sum(debit) debit, sum(credit) credit
FROM masse_be
GROUP BY ide_poste_centra, ide_cpt, libn;
...
select ide_poste_centra poste, ide_cpt compte, libn libelle, debit flux_debit_be, credit flux_credit_be, 0 flux_debit_gestion, 0 flux_credit_gestion , 0 flux_debit_cumul, 0 flux_credit_cumul
from mai_be
union
select ide_poste_centra poste, ide_cpt compte, libn libelle, 0 flux_debit_be, 0 flux_credit_be, debit flux_debit_gestion, credit flux_credit_gestion , 0 flux_debit_cumul, 0 flux_credit_cumul
from mai_gest
union
select ide_poste_centra poste, ide_cpt compte, libn libelle, 0 flux_debit_be, 0 flux_credit_be, 0 flux_debit_gestion, 0 flux_credit_gestion , debit flux_debit_cumul, credit flux_credit_cumul
from masse_cumul;
...
SELECT POSTE, b.LIBN, COMPTE, LIBELLE, FLUX_DEBIT_BE, FLUX_CREDIT_BE, CASE
                                                                        WHEN solde_be > 0 THEN solde_be
                                                                        WHEN solde_be < 0 THEN 0
                                                                        ELSE 0
                                                                    END as DEBIT_BE,
                                                                    CASE
                                                                        WHEN solde_be < 0 THEN ABS(solde_be)
                                                                        WHEN solde_gestion > 0 THEN 0
                                                                        ELSE 0
                                                                    END as CREDIT_BE,
                                                                    FLUX_DEBIT_GESTION, FLUX_CREDIT_GESTION,
                                                                    CASE
                                                                    ...
                                                                    CASE
                                                                        ...
                                                                        ELSE 0
                                                                    END as CREDIT_GEST,
                                                                    FLUX_DEBIT_CUMUL, FLUX_CREDIT_CUMUL,
                                                                    ...
                                                                    CASE
                                                                        ...
                                                                        ELSE 0
                                                                    END as CREDIT,
                                                                    cod_sens_solde sens_normal_compte
...
SELECT POSTE, b.LIBN, COMPTE, LIBELLE, FLUX_DEBIT_BE BE_DB, FLUX_CREDIT_BE BE_CR, FLUX_DEBIT_CUMUL FLUX_DB, FLUX_CREDIT_CUMUL FLUX_CR,
