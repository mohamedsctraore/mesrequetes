drop table cipd_cipr_auj;

create table cipd_cipr_auj as
...
drop table cipd_cipr_2_mois;

create table cipd_cipr_2_mois as
select ide_poste pc_code, b.libn libelle_poste, a.ide_cpt compte, c.libn libelle_compte,    case 
                                                                                                when sum(decode(cod_sens, 'D', mt, -mt)) > 0
                                                                                                then sum(decode(cod_sens, 'D', mt, -mt))
                                                                                                else 0
...
and (substr(ide_cpt, 1,3) = '473' or substr(ide_cpt, 1,3) = '478')
--and ide_poste = '603'
and dat_ecr <= '31/10/2023'
group by ide_poste, b.libn, a.ide_cpt, c.libn
having sum(decode(cod_sens, 'D', mt, -mt)) <> 0
order by a.ide_cpt, a.ide_poste
;

select * from cipd_cipr_2_mois;

select pc_code, libelle_poste, compte, libelle_compte, debit, credit from cipd_cipr_auj a
where (pc_code, libelle_poste, compte)
in
(
select pc_code, libelle_poste, compte
from cipd_cipr_2_mois
where debit - credit <> 0
)
order by compte, pc_code
;